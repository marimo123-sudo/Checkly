...
function renderReviews(containerId, reviews, is_product=false) {
    ...
    reviews.forEach(review => {
        ...
        const timeAgo = review.created_at_ago;
        const reviewHTML = `
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (product_id) {
        try {
            ...
            var wrapper_1 = document.getElementById("first_tab");
...
function showCopied() {
    ...
    if (what_is_is == "me") {
        var text_for_copy = `Привет! Буду рад, если оставишь отзыв обо мне 🙌 Я использую Check’ly — сервис для отзывов, чтобы подтвердить свою репутацию и избежать скама. Вот моя ссылка:\n`
    }
    else if (what_is_is == "product") {
        
        var text_for_copy = `Check’ly — отзывы о каналах, чатах и пользователях. \nСмотри или оставь отзыв:\n`
    }
    else {
        var text_for_copy = `Твой отзыв поможет этому пользователю поднять рейтинг в Check’ly\n`
    }
